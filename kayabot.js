...
(function (f, g) {
  ...
  function ha() {
    ...
    0 != F && (null == ba && (ba = new ca(24, '#FFFFFF')), ba.setValue('Score: ' + ~~(F / 100) + ' || En iyi Skor: ' + ~~(sessionScore / 100) + " || Ceviri By www.kayagray.com | Kaya Giray || En Uzun Yasam Süresi: " + bestTime + " seconds"), c = ba.render(), b = c.width, e.globalAlpha = 0.2, e.fillStyle = '#000000', e.fillRect(10, r - 10 - 24 - 10, b + 10, 34), e.globalAlpha = 1, e.drawImage(c, 15, r - 10 - 24 - 5));
    Na();
    a = + new Date - a;
  ...
  function customRender(d) {
    ...
    var debugStrings = [];
    debugStrings.push("T - Bot Durumu: " + (!toggle ? "Aktif" : "Deaktif"));
    debugStrings.push("R - Grafikler: " + (!toggleDraw ? "Acik" : "Kapali"));
    debugStrings.push("Sunucu: " + serverIP);
    debugStrings.push("Yasam Suresi: " + nbSeconds + " saniye");

    if (getPlayer().length > 0) {
        debugStrings.push("Koordinatlar: " + Math.floor(getPlayer()[0].x) + ", " + Math.floor(getPlayer()[0].y));
    }
